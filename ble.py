...
async def notification_handler_1(sender, data: bytearray):
    ...
    with open("backData.txt", "a") as file:
        file.write(msg + "\n")
    if ready:
        sio.emit("ble_data", {"device": "BackDevice", "errorCode": error_code})
    else:
        print("SocketIO not ready, skipping emit")
    back_error = 1
...
async def notification_handler_2(sender, data: bytearray):
    ...
    with open("wristData.txt", "a") as file:
        file.write(msg + "\n")
    if ready:
        sio.emit("ble_data", {"device": "WristDevice", "errorCode": error_code})
    else:
        print("SocketIO not ready, skipping emit")
    wrist_error = 1
...
async def notification_handler_3(sender, data: bytearray):
    ...
    with open("bicepData.txt", "a") as file:
        file.write(msg + "\n")
    if ready:
        sio.emit("ble_data", {"device": "BicepDevice", "errorCode": error_code})
    else:
        print("SocketIO not ready, skipping emit")
    bicep_error = 1
